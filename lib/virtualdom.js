...
function virtualdom(spec) {
   ...
   * @private
   */
  let selection = selectionObserver(spec);
 ...
 * @return {Observe} Selection observer
 */
function selectionObserver(spec) {
  let selection           = observe([]);
  let manageSelectionWith = R.curry(manageSelection);

  return selection.on('change', manageSelectionWith(spec, selection));
}

/**
 * Manages selection.
 * @param  {object}  spec      - Specifications object
 * @param  {Observe} selection - Observer
 * @param  {object}  event     - Event object
 * @return {Observe} Observer
 */
function manageSelection(spec, selection, event) {
  if (isNotAdded(event) || isIncomplete(selection)) {
    return;
  }

  if (R.equals(first(selection), last(selection))) {
    return selection.splice(0);
  }

  if (R.has('onSelect', spec)) {
    R.prop('onSelect', spec)(subject(selection));
  }
