...
function virtualdom(spec = {}) {
  ...
  function pieces(data) {
    ...
    const ham       = R.ifElse(R.propEq('disabled', false), hammer, R.always({}));

    return h(CONTAINER_CLASS, ham(data), children(data));
  }
  ...
  function hammer(data) {
    return {
      hammer: new Hammer({ events: { tap: selection.withIndex(data) } })
    };
  }
}
