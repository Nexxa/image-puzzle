...
function withPosition(position) {
  return _.set({}, KEYS.POSITION, position);
}
...
function withIndex(piece, index) {
  return _.set(piece, KEYS.INDEX, index + 1);
}
...
function withRow(rowIndex, piece) {
  return _.set(piece, KEYS.ROW, rowIndex + 1);
}
...
function withCol(piece, colIndex) {
  return _.set(piece, KEYS.COL, colIndex + 1);
}
...
function withCoords(piece) {
  let coords = {};
  coords[KEYS.X] = (piece.width * (piece.col - 1));
  coords[KEYS.Y] = (piece.height * (piece.row - 1));

  return _.merge(piece, coords);
}
