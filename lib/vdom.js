...
function update(data) {
  let updated = pieces(data);
  let patches = diff(tree, updated);
...
// -----------------
/**
 * @property {VTree} [tree=null] - Virtual tree of puzzle pieces.
 * @public
 */
let tree = null;
/**
 * @property {DOMNode} [root=null] - Real DOM element equivalent to virtual tree.
 * @public
 */
let root = null;
...
function pieces(data) {
  let asPieceWithSrc = pieceCy(src(data));
  let children       = R.compose(R.map(asPieceWithSrc), pairs);

  return h(CONTAINER_CLASS, children(data));
}
